...
function carregarPedidos() {
    fetch('http://localhost:5285/api/pedidos/cozinha')
    .then(res => res.json())
    .then(data => {
        var tabelaCozinha = document.getElementById('tabela-cozinha')
        tabelaCozinha.innerHTML = ''
        ...
        data.forEach(pedido => {
            ...
            .then(itens => {
                ...
                var linhaTabelaCozinha = document.createElement('tr')
                linhaTabelaCozinha.innerHTML = `
                    <td>${pedido.id}</td>
                ...
                `
                tabelaCozinha.appendChild(linhaTabelaCozinha)
            })
        })
    })
}
